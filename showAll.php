<?php
    ...
    $query = "select name from employee group by name order by EnNo";
    ...
    <div class="wrapper">
        ...
        <div id="content">
           ...
           <table id="showAllTable">
                ...
                <tbody>
                    <?php
                        while($row_name = $result -> fetch_assoc())
                        {
                            $name_result = $row_name['name'];
                            //echo $name_result;

                            $show_all = "Select clock_in.No,clock_in.Name, (clock_in.DateTime) as _in, (clock_out.DateTime) as _out , time(clock_in.DateTime), time(clock_out.DateTime) From clock_in Inner Join clock_out On clock_in.No = clock_out.No where clock_in.Name = '$name_result' order by clock_in.DateTime";

                            $all_result = mysqli_query($connect, $show_all);
                            ...
                            while($row_all_result = $all_result -> fetch_assoc())
                            {
                                $in = $row_all_result['_in'];
                                $out = $row_all_result['_out'];
                                $_timeIn = $row_all_result['time(clock_in.DateTime)'];
                                $_timeOut = $row_all_result['time(clock_out.DateTime)'];

                                $time_in = strtotime($in);
                                $time_out = strtotime($out);
                                $TimeIn = strtotime("_timeIn");
                                $TimeOut = strtotime("_timeOut");
                               ...
                               if($TimeIn > $morning_shift_late || $TimeIn > $afternoon_shift_late || $TimeIn > $night_shift_late)
                                {
                                    if($TimeIn < strtotime('09:30:00'))
                                    {
                                    ...
                                    else if ($TimeIn < strtotime('15:30:00'))
                                    {
                                        $pen_sec = $time_in - $afternoon_shift_late;
                                        $punctual_count ++;
                                    }
                                    else if ($TimeIn < strtotime('23:30:00'))
                                    {
                                ...
                                //      <td>".$name_result."</td>
                                //      <td></td>
                                //      <td style='color:red'>".$total_late_pen." ".$num."</td>
                                //      <td style='color:red'>".$display_pen_shift."</td>
                                //      <td style='color:green'>".$display_bonus."</td>
